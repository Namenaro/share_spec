...
class Visualizer:
    ...
    def _visualise_unsertainty(self, ax, x1, x2, unsertainties):
        ...
        contour = ax.contourf(x1, x2, unsertainties.reshape(100, 100), cmap=cmap)
    ...
    def _visualise_propbability(self, ax, x1, x2, probabilities):
        ...
        contour = ax.contourf(x1, x2, probabilities.reshape(100, 100), cmap=cmap)
        cbar = plt.colorbar(contour, ax=ax)
    ...
    def visualise_specialisations(self, realX, realY, x1, x2, arr_probabilities, arr_unsertainties, unsertainty_threshold, directory):
        ...
        for n in range(num_modules):
            for i in range(len(x1)):
                answer_of_module = arr_probabilities[n][i]
                unsertainty_of_module = arr_unsertainties[n][i]
                if unsertainty_of_module > unsertainty_threshold:
                    if(result_prob[i] < answer_of_module):
                        result_prob[i] = answer_of_module
        #отрисовка ответа ансамбля
        cmap = sns.diverging_palette(250, 12, s=85, l=25, as_cmap=True)
        contour = ax_prob.contourf(x1, x2, result_prob.reshape(100, 100), cmap=cmap)
        ...
        for n in range(num_modules):
            for i in range(len(x1)):
                unsertainty_of_module = arr_unsertainties[n][i]
                if(result_unsert[i] < answer_of_module):
                    result_unsert[i] = answer_of_module
        cmap = sns.cubehelix_palette(light=1, as_cmap=True)
        contour = ax_unsert.contourf(x1, x2, result_unsert.reshape(100, 100), cmap=cmap)
    ...
    def visualise_model(self, realX, realY, x1, x2, unsertainties, probabilities, directory=None):
